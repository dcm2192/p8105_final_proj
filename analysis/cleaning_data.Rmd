...
elk = 
  ...
  dplyr::mutate(
    ...
    dist = 
      ifelse(
        elk_id == lag(elk_id),
        geosphere::distHaversine(cbind(long, lat), cbind(lag(long), lag(lat))),
        NA)
    ) |> 
  dplyr::select(
    elk_id,
...
#land cover data is added below!
...
```{r}
land_cover = raster::raster('../raw_data/land_cover/land_cover.tif')
```
...
Transforming raster coordinates into latitude and longitude
```{r}
library(raster)
library(sf)
library(terra)      
...
```{r}
small_land_coord = crop(land_coord, extent(lowerleftlon, upperrightlon, lowerleftlat, upperrightlat))
```
...
```{r}
test_rast = rast('../clean_data/land_cover.tif')
```
